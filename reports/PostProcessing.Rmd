...
fns_split = str_split_fixed(fns,'[_-\ ]',Inf) %>% 
  ...
  setNames(c('sampleid','experiment','yr','mth','dy','time','imageid')) %>% 
  mutate(date = as.Date(paste(yr, mth, dy,sep='-')),
         datetime = as.POSIXct(paste(date,time),'%Y-%m-%d %H_%M_%S', tz='UTC')) %>% 
  select(sampleid, date, datetime)
...
We need to test if all the ROI are present and distinct. If the plants merged together, then the plant area will be the same for both roi of a sampleid. If 1 plant died then there should only be 1 roi for a sampleid.
...
missingdF %>%
  ggplot(aes(x = as.POSIXct(strptime(timestamp,'%Y-%m-%d %H_%M_%S')), y = plantarea)) +
  geom_line(
...
```{r , include=F}
fns = dir(file.path('data','raw_snapshots','psII'))
...
fns_split = str_split_fixed(fns,'[-\ ]',Inf) %>%
  ...
  mutate(date = as.Date(paste(yr, mth, dy,sep='-')),
         datetime = as.POSIXct(paste(date,time),'%Y-%m-%d %H_%M_%S', tz='UTC'),
         imageid = as.integer(map_chr(.x=str_split(imageid,'_'), .f = pluck, 2))) %>%
  select(sampleid, date, datetime, imageid)
...
```{r echo=T}
n_param = 12 #there are n photosynthetic parameters -->
...
```{r, eval=T}
n_gtyperesults <- n_sampleids/n_gtypes*n_roi*(n_param) #below we filter the dataframe down to 1 output perdistinct sampleid/gtype/parameter
```
